...
function CalcSize_GridView(def, style, line_heights)
    ...
    this.Calculate_Size(def.headers, def.headers, def.sizes.headers)
    ...
    for i = 1, #def.headers do
        def.sizes.final_columns[i] = this.GetFinalWidth_Column(def.headers[i], def.sizes, i)
    end

    -- Final height of each line
    def.sizes.final_row_header = this.GetFinalHeight_Row(def.sizes.headers, style.gridview.min_row_height)

    for i = 1, #def.sizes.cells do
        def.sizes.final_rows_cells[i] = this.GetFinalHeight_Row(def.sizes.cells[i], style.gridview.min_row_height)
    end

    def.render_pos.width = this.GetFinalWidth(def.sizes.final_columns, style.gridview.gap_horizontal)
    def.render_pos.height = this.GetFinalHeight(def.sizes, style.gridview.gap_vertical)
end

function Draw_GridView(def, style_grid, style_colors, const)
...
function this.Calculate_Size(headers, row, list)
    local width = 0
    local height = 0
    ...
    for i = 1, #headers do
        if i <= #row and row[i].text then
            if headers[i].max_width then
                width, height = ImGui.CalcTextSize(row[i].text, false, headers[i].max_width)        -- possible word wrapping
            else
...
function this.GetFinalWidth_Column(header, sizes, col)
    --NOTE: max_width is already accounted for in headers.width and cells.width

    local retVal = sizes.headers[col].width
    ...
    if header.min_width then
        retVal = math.min(retVal, header.min_width)
    end
...
function this.GetFinalHeight_Row(row, min)
    local retVal = 0
