...
function Draw_StickFigure(def, style_graphics, screenOffset_x, screenOffset_y, parent_width, parent_height, const)
    ...
    Draw_Circle(screenOffset_x, screenOffset_y, center_x, top + radius, radius, false, nil, nil, style_graphics.stickfigure_color_abgr, nil, style_graphics.line_thickness_main)

    -- Body
    local pelvis_y = top + (def.height * 0.60632)
    Draw_Line(screenOffset_x, screenOffset_y, center_x, top + (radius * 2), center_x, pelvis_y, style_graphics.stickfigure_color_abgr, style_graphics.line_thickness_main)

    -- Arms
    local arm_y = top + (def.height * 0.37895)
    Draw_Line(screenOffset_x, screenOffset_y, left, arm_y, right, arm_y, style_graphics.stickfigure_color_abgr, style_graphics.line_thickness_main)

    -- Left Leg
    local halfStance = def.width * 0.41729
    Draw_Line(screenOffset_x, screenOffset_y, center_x - halfStance, bottom, center_x, pelvis_y, style_graphics.stickfigure_color_abgr, style_graphics.line_thickness_main)
