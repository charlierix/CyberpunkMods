...
registerHotkey("WallHangTesterButton", "tester hotkey", function()
    ...
    local anim = AnimationCurve:new()

    local hard_coded =
    {
        Vector4.new(-0.24485917389393, 1.7146737575531, 0, 1),
        Vector4.new(3.1759088039398, 15.488067626953, 0, 1),
        Vector4.new(-2.7535283565521, 14.716904640198, 0, 1),
        Vector4.new(7.8685173988342, -2.3514406681061, 0, 1),
        Vector4.new(-0.10773362219334, -1.188885807991, 0, 1),
    }

    for i = 1, 5, 1 do
    --for i = 1, #hard_coded, 1 do
        local point = GetRandomVector_Circular(0, 16)
        --local point = hard_coded[i]

        log:Add_Dot(point, nil, "000", 0.5)

        anim:AddKeyValue(point.x, point.y)
    end

    local min = anim:Min_Key() - 1
    local max = anim:Max_Key() + 1
    ...
    for i = 1, steps, 1 do
        local x = GetScaledValue(min, max, 0, steps - 1, i - 1);
        local y = anim:Evaluate(x)

        table.insert(points, Vector4.new(x, y, 0, 1))
    end

    local keyval_points = Select(anim.keyvalues, function(o) return Vector4.new(o.key, o.value, 0, 1) end)
    for i = 1, #keyval_points - 1, 1 do
        log:Add_Line(keyval_points[i], keyval_points[i+1], nil, "000", 0.25)
    end

    for i = 1, #points - 1, 1 do
        log:Add_Line(points[i], points[i+1], nil, "FFF")
    end

    local bezier_points = GetBezierPoints_Segments(144, anim:Bezier())
    for i = 1, #bezier_points - 1, 1 do
        log:Add_Line(bezier_points[i], bezier_points[i+1], nil, "1E90FF", 0.5)
    end

    for i = 1, #anim.keyvalues, 1 do
        log:Add_Dot(Vector4.new(anim.keyvalues[i].key, anim:Evaluate(anim.keyvalues[i].key), 0, 1), nil, "00F")
    end

    local control_points = SelectMany(anim:Bezier(), function(o) return o.control_points end)
    for i = 1, #control_points, 1 do
        log:Add_Dot(control_points[i], nil, "CD5C5C")
    end

    log:Save("animation curve")
end)
