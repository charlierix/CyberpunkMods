...
function CalcSize_ListBox(def, style, const, line_heights)
...
function Draw_ListBox(def, style_list, em)
    --TODO: Return a selection changed bool (look for when the index changes after drawing a selectable)

    ImGui.PushStyleVar(ImGuiStyleVar.FramePadding, style_list.padding * em, style_list.padding * em)
    ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, style_list.border_cornerRadius * em)
    ...
    if ImGui.BeginListBox("##" .. def.invisible_name, def.width * em, def.height * em) then
        if def.items then
