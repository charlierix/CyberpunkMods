...
function CalcSize_OrderedList(def, style, const, line_heights)
	if not def.sizes then
        def.sizes = {}
    end

    local width_p, width_g, width_v, width_total = this.Calculate_Width(def, line_heights.line)
...
function this.Calculate_Width(def, em)
    local prompt = 0
    local gap = 0
    local value = 0

    for _, content in pairs(def.content) do
        local p, g, v = this.Calculate_Width_PromptValue(content.prompt, content.value, def.gap * em)
