...
-- This will set the property to this wall hang mod's string
function Custom_CurrentlyFlying_StartFlight(player, modNames)
    pcall(function(p, mn)
        p.Custom_CurrentlyFlying = mn.wall_hang
    end, player, modNames)
end

-- This will clear the property only if wall hang is still current
function Custom_CurrentlyFlying_Clear(player, modNames)
    pcall(function(p, mn)
        local current = p.Custom_CurrentlyFlying

        if current == mn.wall_hang then
