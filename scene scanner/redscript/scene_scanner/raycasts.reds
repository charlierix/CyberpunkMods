...
public func SceneScanner_RayCast(from: Vector4, to: Vector4, vehicles: Bool) -> TraceResult {
    ...
    if spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Static", attempt) {
        ...
        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("SceneScanner_RayCast: static");
        };
    };
    ...
    if spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Dynamic", attempt) {
        attemptDistSqr = SceneScanner_LenSqr(from, attempt.position);

        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("SceneScanner_RayCast: dynamic");
        };
    };

    if vehicles && spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Vehicle", attempt) {
        attemptDistSqr = SceneScanner_LenSqr(from, attempt.position);
...
private func SceneScanner_LenSqr(from: Vector4, to: Vector3) -> Float
{
    return
...
// @addMethod(PlayerPuppet)
// public func SceneScanner_RayCast_Position_ATTEMPTS(from: Vector4, to: Vector4, staticOnly: Bool) -> String {
//     let filter: CName;
//     if staticOnly {
