...
public func Airplane_RayCast(from: Vector4, to: Vector4, staticOnly: Bool) -> TraceResult {
    ...
    if spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Static", attempt) {
        attemptDistSqr = Airplane_LenSqr(from, attempt.position);

        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("Airplane_RayCast: static");
        };
    };
    ...
    //         distSqr = attemptDistSqr;
    //         result = attempt;
    //         Log("Airplane_RayCast: Simple Environment Collision");
    //     };
    // };
...
private func Airplane_LenSqr(from: Vector4, to: Vector3) -> Float
{
    return
...
// @addMethod(PlayerPuppet)
// public func Airplane_RayCast_Position_ATTEMPTS(from: Vector4, to: Vector4, staticOnly: Bool) -> String {
//     let filter: CName;
//     if staticOnly {
