...
function FloatPlayer_GetAcceleration(storage, pos, const)
    ...
    for i=1, nearbyPoints:GetCount() do
        ...
        local lin_x, lin_y, lin_z  = FloatPlayer_GetRepulseAccel_Linear(direction, distance, const.linear_maxDist, const.linear_maxAccel)
        x = x + lin_x
        y = y + lin_y
        z = z + lin_z

        local inv_x, inv_y, inv_z = FloatPlayer_GetRepulseAccel_Inverse(direction, distance, const.inverse_maxDist, const.inverse_maxAccel, const.inverse_c);
        x = x + inv_x
        y = y + inv_y
        z = z + inv_z

        local sqr_x, sqr_y, sqr_z = FloatPlayer_GetRepulseAccel_InvSqr(direction, distance, const.inverseSqr_maxDist, const.inverseSqr_maxAccel, const.inverseSqr_c)
        x = x + sqr_x
        y = y + sqr_y
        z = z + sqr_z
    end
...
function FloatPlayer_GetRepulseAccel_Linear(directionUnit, distance, maxDist, maxAccel)
...
function FloatPlayer_GetRepulseAccel_Inverse(directionUnit, distance, maxDist, maxAccel, constant)
    if distance > maxDist then
        return 0, 0, 0
    end
...
-- Force is 1/x^2
function FloatPlayer_GetRepulseAccel_InvSqr(directionUnit, distance, maxDist, maxAccel, constant)
    if distance > maxDist then
        return 0, 0, 0
    end
