...
public func LowFlyingV_RayCast(from: Vector4, to: Vector4, staticOnly: Bool) -> TraceResult {
    ...
    if spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Static", attempt) {
        attemptDistSqr = LowFlyingV_LenSqr(from, attempt.position);

        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("LowFlyingV_RayCast: static");
        };
    };
    ...
    if !staticOnly && spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Dynamic", attempt) {
        ...
        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("LowFlyingV_RayCast: dynamic");
        };
    };
    ...
    if !staticOnly && spacialQuery.SyncRaycastByCollisionGroup(from, to, n"Vehicle", attempt) {
        ...
        if distSqr < 0.00 || attemptDistSqr < distSqr {
            distSqr = attemptDistSqr;
            result = attempt;
            //Log("LowFlyingV_RayCast: vehicle");
        };
    };
    ...
    //         distSqr = attemptDistSqr;
    //         result = attempt;
    //         Log("LowFlyingV_RayCast: Simple Environment Collision");
    //     };
    // };
...
private func LowFlyingV_LenSqr(from: Vector4, to: Vector3) -> Float
{
    return
...
// @addMethod(PlayerPuppet)
// public func LowFlyingV_RayCast_Position_ATTEMPTS(from: Vector4, to: Vector4, staticOnly: Bool) -> String {
//     let filter: CName;
//     if staticOnly {
