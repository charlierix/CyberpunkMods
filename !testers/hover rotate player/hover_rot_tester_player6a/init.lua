--===========================================================================
-- Hover Rot Tester Player 6a — PSM State Machine Manipulation + Teleport
--
-- Copy of tester 6 with fixes:
--   1. RE-TELEPORT BUG FIXED: teleport back to state.targetZ (not falling pos.z)
--      This was the critical bug that kept player at ground level, prevented
--      DIAG output, and made all results inconclusive.
...
--   3. All names/IDs updated from Player6/HRTP6 to Player6a/HRTP6A
--
...
--
-- Hotkeys (bind in Settings > Key Bindings > HoverRotTesterPlayer6a):
--   1. Toggle Hover (Player6a)       — enable/disable the tester
--   2. Cycle State Mode (Player6a)   — cycle through PSM state modes
--   3. Toggle Rotation Method (P6a)  — switch between Teleport and SetWorldTransform
--   4. Dump PSM State (Player6a)     — print all PSM blackboard values
--   5. Probe SM Component (Player6a)— dump gamestateMachineComponent info
--   6. Yaw +30 (Player6a)           — rotate yaw +ROT_STEP
--   7. Yaw -30 (Player6a)           — rotate yaw -ROT_STEP
--   8. Pitch +30 (Player6a)         — rotate pitch +ROT_STEP
--   9. Pitch -30 (Player6a)         — rotate pitch -ROT_STEP
--  10. Roll +30 (Player6a)          — rotate roll +ROT_STEP
--  11. Roll -30 (Player6a)          — rotate roll -ROT_STEP
--
-- Install: Copy to bin/x64/plugins/cyber_engine_tweaks/mods/hover_rot_tester_player6a
--===========================================================================

local ModName  = "HoverRotTesterPlayer6a"
...
local function activate()
    ...
    print(string.format("[%s] Re-teleport when z < %.1f — FIXED: will teleport back to z=%.1f", ModName, state.reTeleportThreshold, state.targetZ))
...
registerHotkey("HRTP6A_Toggle", "Toggle Hover (Player6a)", function()
...
registerHotkey("HRTP6A_CycleState", "Cycle State Mode (Player6a)", function()
    cycleStateMode()
end)

registerHotkey("HRTP6A_ToggleMethod", "Toggle Rotation Method (P6a)", function()
    toggleRotationMethod()
end)

registerHotkey("HRTP6A_DumpPSM", "Dump PSM State (Player6a)", function()
    dumpPSM()
end)

registerHotkey("HRTP6A_ProbeSM", "Probe SM Component (Player6a)", function()
    probeSM()
end)

registerHotkey("HRTP6A_YawPos", "Yaw +30 (Player6a)", function()
    applyRotation(0, 0, 1, ROT_STEP)
end)

registerHotkey("HRTP6A_YawNeg", "Yaw -30 (Player6a)", function()
    applyRotation(0, 0, 1, -ROT_STEP)
end)

registerHotkey("HRTP6A_PitchPos", "Pitch +30 (Player6a)", function()
    applyRotation(1, 0, 0, ROT_STEP)
end)

registerHotkey("HRTP6A_PitchNeg", "Pitch -30 (Player6a)", function()
    applyRotation(1, 0, 0, -ROT_STEP)
end)

registerHotkey("HRTP6A_RollPos", "Roll +30 (Player6a)", function()
    applyRotation(0, 1, 0, ROT_STEP)
end)

registerHotkey("HRTP6A_RollNeg", "Roll -30 (Player6a)", function()
    applyRotation(0, 1, 0, -ROT_STEP)
end)
...
registerForEvent("onInit", function()
    ...
    print(string.format("[%s] Fixed version of tester 6: re-teleport to targetZ + improved SM identifier probing", ModName))
...
registerForEvent("onUpdate", function(delta)
    ...
    -- FIX: teleport back to state.targetZ (not falling pos.z)
    -----------------------------------------------------------------------
    ...
    if pos.z < state.reTeleportThreshold then
        ...
        if shouldLog then
            print(string.format("[%s] z=%.1f below threshold %.1f — will teleport to z=%.1f (count=%d)",
        ...
        -- No separate re-teleport here — the rotation teleport below will
        -- use targetZ for z, bringing the player back up with full rotation
        -- in a single teleport per frame (fixes double-teleport bug)
    else
    ...
    -- Use targetZ for z so the single teleport per frame keeps the player
    -- airborne AND applies full rotation (fixes double-teleport bug where
    -- the re-teleport and rotation teleport fought each other)
