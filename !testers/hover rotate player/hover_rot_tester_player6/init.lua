--===========================================================================
-- Hover Rot Tester Player 6 — PSM State Machine Manipulation + Teleport
--
-- Steps 3 & 7 from testers/hover_rot_tester_player3/next steps.md:
...
--
-- Teleport was shown to set yaw (tester 4) but roll/pitch get clamped by
-- locomotion afterward. The question is: does a different PSM state skip
-- that clamp?
--
-- This tester also probes the gamestateMachineComponent itself via
-- FindComponentByType and GetSnapshotContainer to understand the SM structure.
--
...
--
-- Hotkeys (bind in Settings > Key Bindings > HoverRotTesterPlayer6):
--   1. Toggle Hover (Player6)       — enable/disable the tester
--   2. Cycle State Mode (Player6)   — cycle through PSM state modes
--   3. Toggle Rotation Method (P6)  — switch between Teleport and SetWorldTransform
--   4. Dump PSM State (Player6)     — print all PSM blackboard values
--   5. Probe SM Component (Player6)— dump gamestateMachineComponent info
--   6. Yaw +30 (Player6)           — rotate yaw +ROT_STEP
--   7. Yaw -30 (Player6)           — rotate yaw -ROT_STEP
--   8. Pitch +30 (Player6)         — rotate pitch +ROT_STEP
--   9. Pitch -30 (Player6)         — rotate pitch -ROT_STEP
--  10. Roll +30 (Player6)          — rotate roll +ROT_STEP
--  11. Roll -30 (Player6)          — rotate roll -ROT_STEP
--
-- Install: Copy to bin/x64/plugins/cyber_engine_tweaks/mods/hover_rot_tester_player6
--===========================================================================

local ModName  = "HoverRotTesterPlayer6"
local ROT_STEP = 30    -- degrees per rotation press
local NUM_DIAG = 5     -- frames of debug output after a change
...
local function savePSMValues(psmBB, psmDef)
    ...
    local intVars = {
        ...
        "Takedown", "Fall", "Landing", "Melee",
    }
    local boolVars = {
        "Felled", "MountedToVehicle", "MountedToVehicleInDriverSeat",
        "IsInWorkspot", "IsInteractingWithDevice", "IsOnGround",
...
local function probeSMComponent(player)
    ...
    if sm then
        ...
        -- Try IsStateMachinePresent for known state machines
        ...
        for _, smName in ipairs(knownSMs) do
            ...
            pcall(function()
                ...
                present = sm:IsStateMachinePresent(smid)
            end)
            if present ~= nil then
                print(string.format("[%s]   IsStateMachinePresent('%s') = %s", ModName, smName, tostring(present)))
            end
        end
    else
...
registerHotkey("HRTP6_Toggle", "Toggle Hover (Player6)", function()
...
registerHotkey("HRTP6_CycleState", "Cycle State Mode (Player6)", function()
    cycleStateMode()
end)

registerHotkey("HRTP6_ToggleMethod", "Toggle Rotation Method (P6)", function()
    toggleRotationMethod()
end)

registerHotkey("HRTP6_DumpPSM", "Dump PSM State (Player6)", function()
    dumpPSM()
end)

registerHotkey("HRTP6_ProbeSM", "Probe SM Component (Player6)", function()
    probeSM()
end)

registerHotkey("HRTP6_YawPos", "Yaw +30 (Player6)", function()
    applyRotation(0, 0, 1, ROT_STEP)
end)

registerHotkey("HRTP6_YawNeg", "Yaw -30 (Player6)", function()
    applyRotation(0, 0, 1, -ROT_STEP)
end)

registerHotkey("HRTP6_PitchPos", "Pitch +30 (Player6)", function()
    applyRotation(1, 0, 0, ROT_STEP)
end)

registerHotkey("HRTP6_PitchNeg", "Pitch -30 (Player6)", function()
    applyRotation(1, 0, 0, -ROT_STEP)
end)

registerHotkey("HRTP6_RollPos", "Roll +30 (Player6)", function()
    applyRotation(0, 1, 0, ROT_STEP)
end)

registerHotkey("HRTP6_RollNeg", "Roll -30 (Player6)", function()
    applyRotation(0, 1, 0, -ROT_STEP)
end)
...
registerForEvent("onInit", function()
    ...
    print(string.format("[%s] Steps 3 & 7: PSM blackboard manipulation + SM component probe", ModName))
...
registerForEvent("onUpdate", function(delta)
    ...
    if pos.z < state.reTeleportThreshold then
        ...
        if shouldLog then
            print(string.format("[%s] z=%.1f below threshold %.1f — re-teleporting (count=%d)",
                ModName, pos.z, state.reTeleportThreshold, state.reTeleportCount))
    ...
    if printDiag then
        ...
        print(string.format("[%s] DIAG mode=%s method=%s", ModName, mode.name, state.useTeleport and "Teleport" or "SWT"))
    ...
    if state.useTeleport then
        ok, err = teleportWithOrientation(player, pos, roll, pitch, yaw)
    else
        ok, err = setWorldTransformWithOrientation(player, pos, roll, pitch, yaw)
    end
