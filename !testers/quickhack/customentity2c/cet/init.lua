...
-- OrbHackingBridge Quickhack Test -- Quickhack Clearance Fix + Action Dump
-- ============================================================
--
-- Purpose: Test the REDscript OrbHackingBridge bridge with the
--   quickhack-specific clearance fix. CE2b proved the bridge
--   loads successfully (OnAttach/OnDetach fix worked), but both
--   phases returned NO_ACTION because the bridge used
--   Device.GetInteractionClearance() instead of
--   Device.GetInteractionQuickHackClearance().
--
--   CE2c fixes:
--     1. REDscript: GetInteractionClearance -> GetInteractionQuickHackClearance
--     2. CET: Fixed diagnostic line that crashed onInit (Game.GetScriptableSystem
--        access threw error because the function doesn't exist on GameInstance)
--     3. CET: Added action dump logging before execution
--     4. CET: Added fallback to ExecuteFirstAvailableAction if specific action
--        returns NO_ACTION
--     5. CET: Added third hotkey to list available actions on targeted device
--
--   Uses phased testing:
--     Phase 1: Player as executor (proves bridge + SetUp + native pipeline)
--     Phase 2: Drone as executor (tests IsPossible gate)
--
-- Ships its own modified REDscript (OrbHackingBridge.reds) with
-- quickhack clearance fix. Red4ext is reused from customentity2.
--
-- File: init.lua
-- Install: bin/x64/plugins/cyber_engine_tweaks/mods/customentity2c/init.lua
-- Target: Cyberpunk 2077 v2.2+
-- Requires: CET 1.39.1+, REDscript, RED4ext (from customentity2)
--
-- Hotkeys (bind in Settings > Key Bindings):
--   CE2c: Spawn/Despawn Drone
--   CE2c: Run Ping Quickhack Test
--   CE2c: List Device Actions
--
-- ============================================================

local MOD_NAME = 'customentity2c'
local BRIDGE_NAME = 'OrbHackingBridge'
local LOG_PREFIX = '[CE2c]'
...
local function RunPingTest()
    ...
    if not bridge then
        Log('ERROR: OrbHackingBridge not found -- check REDscript deployment')
        Log('Ensure r6/scripts/OrbHackingBridge.reds is the CE2c version (with GetInteractionQuickHackClearance)')
        return
    end
    ...
    -- Try specific action (PingDevice)
    local ok1, result1 = pcall(function()
        return bridge:ExecuteDeviceActionByName(target, "PingDevice", player)
    end)
    if ok1 then
        Log(string.format('Phase 1 result (PingDevice): %s', tostring(result1)))

        -- If NO_ACTION, try fallback: first available action
        if tostring(result1):match('^NO_ACTION') then
            Log('Phase 1 PingDevice not found -- trying first available action as fallback')
    ...
    if state.entity then
        ...
        if ok2 then
            Log(string.format('Phase 2 result (PingDevice): %s', tostring(result2)))

            if tostring(result2):match('^NO_ACTION') then
                Log('Phase 2 PingDevice not found -- trying first available action as fallback')
        ...
        Log('Spawn a drone first with CE2c: Spawn/Despawn Drone, then re-run this test')
    end
...
registerHotkey("CE2c_SpawnDespawnDrone", "CE2c: Spawn/Despawn Drone", function()
    SafeCall('SpawnDrone', SpawnDrone)
end)

registerHotkey("CE2c_RunPingTest", "CE2c: Run Ping Quickhack Test", function()
    SafeCall('RunPingTest', RunPingTest)
end)

registerHotkey("CE2c_ListDeviceActions", "CE2c: List Device Actions", function()
...
registerForEvent("onInit", function()
    ...
    Log('CE2c initialized -- quickhack clearance fix + action dump')
    Log('Fixes: GetInteractionQuickHackClearance + pcall diagnostic + action dump + fallback')
