...
-- OrbHackingBridge Quickhack Test -- REDscript Lifecycle Fix + Phased Testing
-- ============================================================
--
-- Purpose: Test the REDscript OrbHackingBridge bridge with the
--   OnAttach/OnDetach lifecycle methods that were missing in CE2/CE2a.
--   Without these methods, the engine does not instantiate the
--   ScriptableSystem, so GetScriptableSystemsContainer():Get()
--   returns nil even though the class compiles correctly.
--
--   Uses phased testing:
...
-- OnAttach/OnDetach added. Red4ext is reused from customentity2.
--
-- File: init.lua
-- Install: bin/x64/plugins/cyber_engine_tweaks/mods/customentity2b/init.lua
-- Target: Cyberpunk 2077 v2.2+
-- Requires: CET 1.39.1+, REDscript, RED4ext (from customentity2)
--
-- Hotkeys (bind in Settings > Key Bindings):
--   CE2b: Spawn/Despawn Drone
--   CE2b: Run Ping Quickhack Test
--
-- ============================================================

local MOD_NAME = 'customentity2b'
local BRIDGE_NAME = 'OrbHackingBridge'
local LOG_PREFIX = '[CE2b]'
...
local function RunPingTest()
    ...
    -- Get bridge (fixed REDscript with OnAttach/OnDetach)
    local bridge = GetBridge()
    if not bridge then
        Log('ERROR: OrbHackingBridge not found -- check REDscript deployment')
        Log('Ensure r6/scripts/OrbHackingBridge.reds is the CE2b version (with OnAttach/OnDetach)')
        return
    end
    ...
    if state.entity then
        ...
        Log('Spawn a drone first with CE2b: Spawn/Despawn Drone, then re-run this test')
    end
...
registerHotkey("CE2b_SpawnDespawnDrone", "CE2b: Spawn/Despawn Drone", function()
    SafeCall('SpawnDrone', SpawnDrone)
end)

registerHotkey("CE2b_RunPingTest", "CE2b: Run Ping Quickhack Test", function()
...
registerForEvent("onInit", function()
    ...
    Log('CE2b initialized -- REDscript lifecycle fix + phased testing')
    Log('Ships fixed REDscript with OnAttach/OnDetach (required for ScriptableSystem instantiation)')
