...
--
--   CE2e fixes:
--     1. REDscript: GetInteractionClearance -> GetInteractionClearance
...
--
-- File: init.lua
-- Install: bin/x64/plugins/cyber_engine_tweaks/mods/customentity2e/init.lua
-- Target: Cyberpunk 2077 v2.2+
-- Requires: CET 1.39.1+, REDscript, RED4ext (from customentity2)
--
-- Hotkeys (bind in Settings > Key Bindings):
--   CE2e: Spawn/Despawn Drone
--   CE2e: Run Ping Quickhack Test
--   CE2e: List Device Actions
--
-- ============================================================

local MOD_NAME = 'customentity2e'
local BRIDGE_NAME = 'OrbHackingBridge'
local LOG_PREFIX = '[CE2e]'
...
local function RunPingTest()
    ...
    if not bridge then
        Log('ERROR: OrbHackingBridge not found -- check REDscript deployment')
        Log('Ensure r6/scripts/OrbHackingBridge.reds is the CE2e version (with GetInteractionClearance)')
        return
    end
    ...
    if state.entity then
        ...
        Log('Spawn a drone first with CE2e: Spawn/Despawn Drone, then re-run this test')
    end
...
registerHotkey("CE2e_SpawnDespawnDrone", "CE2e: Spawn/Despawn Drone", function()
    SafeCall('SpawnDrone', SpawnDrone)
end)

registerHotkey("CE2e_RunPingTest", "CE2e: Run Ping Quickhack Test", function()
    SafeCall('RunPingTest', RunPingTest)
end)

registerHotkey("CE2e_ListDeviceActions", "CE2e: List Device Actions", function()
...
registerForEvent("onInit", function()
    ...
    Log('CE2e initialized -- quickhack clearance fix + action dump')
